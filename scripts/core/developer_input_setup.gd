...
static func register_playtest_actions() -> void:
	...
	_set_action(InputActions.DRAGON_DAMAGE, [_key(KEY_F8)])
	_set_action(InputActions.DRAGON_HEAL, [_key(KEY_F8, true, false)])
	_set_action(InputActions.DRAGON_FORCE_KO, [_key(KEY_F9)])
	_set_action(InputActions.DRAGON_FORCE_REVIVE, [_key(KEY_F9, true, false)])
